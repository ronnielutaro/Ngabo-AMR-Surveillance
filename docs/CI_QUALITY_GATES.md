...
## Container quality lane (Issue #89)

`.github/workflows/container-quality.yml` builds both production images (`ngabo-core`, `ngabo-web`) on every PR, verifies runtime behavior (core `ngabo-health` output, non-root user, web HTTP smoke, exposed ports), and runs pinned Trivy scans failing on HIGH/CRITICAL findings. It requires **no GCP credentials** and never authenticates to Google Cloud or publishes images. The repository ruleset required checks (`PR Quality Gate`, `CI Control Plane`) remain the merge authority.

Image publishing is owned exclusively by `.github/workflows/publish-containers.yml`: manual `workflow_dispatch` from `refs/heads/develop` in the `dev` GitHub environment, keyless WIF authentication as `ngabo-deployer` (repository-scoped `roles/artifactregistry.writer` on `ngabo-artifacts`), commit-addressable tags with the immutable digest as the authoritative artifact identity (no `latest` promotion identity), and machine-readable publish evidence generated by `infra/github/container_evidence.py`. Base images are pinned by immutable digest in the Dockerfiles.
