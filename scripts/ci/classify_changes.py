...
def classify(paths: Iterable[str]) -> Classification:
    ...
    if not changed:
        return Classification(True, True, True, True, True, True, False)
    ...
    shared = any(path in SHARED_FILES for path in changed) or ci_control
    core = shared or any(path.startswith(CORE_PREFIXES) for path in changed)
    web = shared or any(path.startswith(WEB_PREFIXES) for path in changed)
    ...
    if docs_only:
        core = web = infra = shared = dependency_changed = ci_control = False

    return Classification(
        core, web, infra, shared, dependency_changed, ci_control, docs_only
    )
