...
def verify_upload(
    *,
    ...
    lab_id = headers.get("X-Ngabo-Lab-Id", "")
    source_id = headers.get("X-Ngabo-Source-Id", "")
    timestamp = headers.get("X-Ngabo-Timestamp", "")
    sha256 = headers.get("X-Ngabo-Content-SHA256", "")
    signature = headers.get("X-Ngabo-Signature", "")
    filename = headers.get("X-Ngabo-Filename", "")
