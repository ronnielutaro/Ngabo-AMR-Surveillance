...
class DuplicateRecordFinding:
    ...
    def __post_init__(self) -> None:
        ...
        last_idx = self.original_index
        for idx in self.duplicate_indices:
            if not isinstance(idx, int) or isinstance(idx, bool) or idx <= last_idx:
                raise ValueError(
                    f"Invalid duplicate index {idx!r}; "
                    f"must be strictly increasing integer > {last_idx}"
                )
            last_idx = idx
